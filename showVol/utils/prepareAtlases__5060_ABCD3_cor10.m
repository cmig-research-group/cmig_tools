function prepareAtlases__5060_ABCD3_cor10(atlas_dspace, atlasVersion, source, sourceAnat, varargin)
    ...
    %
    % Nov 2024, update for 6.0 (Note, with new file naming consistencies, this code should 
    % work with new release versions moving forward)
    %
    ...
    % 10/15/2024: remove hard coded paths 

    p = inputParser;
    addRequired(p, 'atlas_dspace', @(x) ischar(x) && ~isempty(x));
    ...
    atlasVersion = p.Results.atlasVersion;
    source = p.Results.source;
    sourceAnat = p.Results.sourceAnat;
    outdir = p.Results.outdir;
    adjustCenter = p.Results.adjustCenter;
    atlases = p.Results.atlases;
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % loop over ABCD ROIs
    % find indices of aseg, aparcaseg and fiber in atlases 
    idx_abcd_rois = find(ismember(atlases, {'aseg','aparcaseg','fiber'}));
    for iA = idx_abcd_rois
        atlas = atlases{iA};
        disp(atlas)
        ...
        % remove all of the aseg rois from aparcaseg. In principle, does fitting 
        % jointly change them from the aseg-only data?
        %  Checking the probMax for aseg ROIs, they are identical in the aseg and 
        % aparcaseg atlases, from which we assume the probs are the same (except 
        % for CC_All and Ventricles_All...)
        ...
        if strcmp(atlas,'aparcaseg')
            ...
            aparcAbbrev = {'bSTS','cACC','cMFC','Cun','Ent','Fus','IPC','ITC','isthCing','lOcc','lOFC',...
                            'Ling','mOFC','mTem','paraHipp','paraCen','POperc','POrb','PTri','periCalc',
                            'postCen','PCC', 'preCen','preCun','rACC','rMFC','SFC','SPC','STC',...
                            'SMar','FP','TP','tTem','Ins'};
