...
%           roiatlas    atlas version for ROIs: ABCD1 or ABCD2 (defaults to ABCD1, as of May 2021)
...
if handles.hasABCDBrain
  if isempty(handles.anat.roifile)
    ...
    if isempty(handles.anat.roiatlas)
      fprintf('%s: Defaulting to ABCD2 atlas ROIs. Set cfg.roiatlas to ''ABCD1'' or specify a specific cfg.roifile if you want something else.\n', mfilename)
      handles.anat.roiatlas = 'ABCD2';
    end
    % default roifile
    if ~isfield(defaultRoifile, handles.anat.roiatlas(1:5))
      error('%s: Invalid cfg.roiatlas (%s) -- only ABCD1 and ABCD2 are supported.', mfilename, handles.anat.roiatlas(1:5))
    end
  ...
  if ~exist(handles.anat.roifile,'file')
    fprintf(2,'%s: ROI file (%s) not found. Make sure you have downloaded showVolData and set the path in ~/abcdConfig.json.',mfilename,  handles.anat.roifile)
    handles.anat.roifile = [];
  end
end
...
if isempty(atlas)
  ...
  if handles.anat.(atlas).Mvxl2lph(1,4) ~= -100, error('Problem with ROI coordinates--make sure you have the latest version of showVolAtlases_1mm.mat'), end
