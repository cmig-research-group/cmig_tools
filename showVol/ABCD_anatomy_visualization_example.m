...
% catch-all example of visualizing ABCD prerendered volumes using showVol

%% select atlas version and load prerendered images and FOD. 
%  If not at UCSD, showVolData files can be downloaded here: 
%  https://drive.google.com/drive/folders/1Uq04VEKRkgQ5SlWDfV9T4KXbwNuCY0nR?usp=sharing TODO: UPDATE
...
loadPrerenderedIfNeeded(atlasVersion, fodtype) %caches prerendered images into global variable PRI on first call. This was most needed with FODs, but saves a little time regardles
global PRI %this global variable contains the loaded prerendered images
...
if contains(atlasVersion, '5.0_ABCD3')
    showVol(atlas_T1('5.0_ABCD3',false), atlas_T1('5.0'), atlas_T1('ABCD2'), atlas_T1('ABCD1'),thalamus_T1,...
      PRI.ABCD3_50.aseg_rgb, PRI.ABCD3_50.aparcaseg_rgb, PRI.ABCD3_50.fiber_rgb, PRI.ABCD3_50.CO, cfg2);
end

if contains(atlasVersion, '6.0_ABCD3')
    showVol(atlas_T1('6.0_ABCD3'), atlas_T1('5.0'), atlas_T1('ABCD2'), atlas_T1('ABCD1'),thalamus_T1,...
      PRI.ABCD3_60.aseg_rgb, PRI.ABCD3_60.aparcaseg_rgb, PRI.ABCD3_60.fiber_rgb, PRI.ABCD3_60.CO, cfg2);
end
