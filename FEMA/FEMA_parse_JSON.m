function [FFX_names, FFX_categorical, FFX_vectorTransforms, ...
          FFX_deltaTransforms, FFX_splineTransforms, FFX_global] = FEMA_parse_JSON(configFile)
% Function that reads a DEAP-created JSON specification files and parses
% the fixed effects information out of it into a standardized structure
% format that can be used by FEMA_makeDesign
%% Input(s):
...
if isfield(cfg_fixed, 'transform_global')
    FFX_global = cfg_fixed.transform_global;
else
    FFX_global = 'none';
end
