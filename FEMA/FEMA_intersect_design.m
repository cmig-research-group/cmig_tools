function [X, iid, eid, fid, agevec, ymat, GRM, PregID, HomeID, info] = FEMA_intersect_design(designMatrix, ymat, iid_concat, eid_concat, varargin)
    ...
    info.provenance = 'FEMA_intersect_design';
    ...
    if iscategorical(eid_design)
        eid_design = cellstr(eid_design);
    end
    ...
    % check if no observations left for analysis
    if numel(iid) == 0
        error('No observations remaining for analysis.');
    end
