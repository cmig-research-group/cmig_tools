function [beta_hat,      beta_se,        zmat,        logpmat,              ...
          ...
          FEMA_fit_firth(X, iid, eid, fid, agevec, ymat, niter, contrasts, nbins, ...
                   pihatmat, varargin)
...
while ~converged && (iter < MaxIter)
    ...
    if iter == 1
        ...
        u        = u_update + (ymat - p + bias_adj) .* W_1;
        ...
        if (norm(beta_hat - beta_old) <= tol) && (norm(sig2mat - sig2mat_old) <= tol)
            converged = true;
        end
