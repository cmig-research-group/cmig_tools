...
%
%% Outputs:
% surf_dbf:     derivatives of spline estimates for different values of xvec
%
% semat_dbf:   standard error of the derivative of the spline estimates for each value of xvec
%
% zmat_dbf:    z-scores of the derivative of the spline estimates
%
% surf_bf:      the spline estimates (not their derivatives) for different values of xvec
%
...
semat_dbf = NaN([length(xvec) size(beta_hat,2)]);
zmat_dbf = NaN([length(xvec) size(beta_hat,2)]);
...
% basis functions
semat_bf = NaN([length(xvec) size(beta_hat,2)]);
zmat_bf = NaN([length(xvec) size(beta_hat,2)]);
for xi = 1:length(xvec)
    wvec = bfmat(xi,:)';
