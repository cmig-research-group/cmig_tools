function [allWsTerms, beta_hat, beta_cov] = FEMA_GLS(y, X, W_1, sig2mat, RandomEffects, ...
...
beta_hat       = zeros(size(X,2), 1, class(y));
beta_cov       = [];
    ...
    if GroupByFamType
        ...
        for fi = 1:nfamtypes
            ...
            for ri = 1:(length(RandomEffects)-1)
                Vs_famtype = Vs_famtype + sig2mat(ri) * currClus{RFX_ord(ri)};
            end

            Vs_famtype = Vs_famtype + currClus{RFX_ord(length(RandomEffects))} .* ...
              diag(W_1(currClus{locJVec})); %jvec_fam
            ...
            if rcond(Vs_double) < eps(class(Vs_double))
                Vis_famtype = cast(pinv(Vs_double), SingleOrDouble);
            else
                Vis_famtype = Vs_double \ eye(tmpSize, SingleOrDouble);
            end
        ...
        allWsTerms = sparse(allR, allC, allV, nobs, nobs, nnz_max);
        ...
        for fi = 1:nfam
            ...
            % Compute V
            for ri = 1:(length(RandomEffects)-1)
                Vs_fam = Vs_fam + sig2mat(ri) * currClus{RFX_ord(ri)};
            end

            Vs_fam = Vs_fam + currClus{RFX_ord(length(RandomEffects))} .* ...
             diag(W_1(currClus{locJVec})); 
            ...
            if rcond(Vs_double) < eps(class(Vs_double))
                Vis_fam = cast(pinv(Vs_double), SingleOrDouble);
            else
                Vis_fam = Vs_double \ eye(tmpSize, SingleOrDouble);
            end
