...
[logLikvec,     beta_hat_perm, beta_se_perm,   zmat_perm, ...
 sig2tvec_perm, sig2mat_perm,  logLikvec_perm, perms] = deal([]); %#ok<*ASGLU>
...
[clusterinfo, Ss, iid, famtypevec, famtypelist, subj_famtypevec] =                  ...
 FEMA_parse_family(iid, eid, fid, agevec, pihatmat, 'RandomEffects', RandomEffects, ...
                   ...
                   'PregID', p.Results.PregID, 'HomeID', p.Results.HomeID);
...
[beta_hat, beta_se, zmat, ymat_hat, ymat_res] = deal(zeros(size(beta_hat), class(ymat)));
...
for coli_ri = 1:ncols_ri
    ...
    for permi = 0:nperms
        ...
        for iter = 1:max(1,niter)
            ...
            if nseg == 1
                [beta_hat, beta_se, betacon_hat, betacon_se] =              ...
                 FEMA_sig2binseg_parfeval(X, ymat, contrasts, clusterinfo,  ...
                                          binvec, sig2mat, sig2tvec,        ...
                                          RandomEffects, GroupByFamType,    ...
                                          nfamtypes, famtypevec, OLSflag,   ...
                                          SingleOrDouble);
