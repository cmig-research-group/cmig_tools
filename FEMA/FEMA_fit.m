...
%
% If you use splines, unstructured covariance, or the GWAS module, please
% additionally cite:
% Parekh et al., (2026) - FEMA-Long: Modeling unstructured covariances for 
%                         discovery of time-dependent effects in large-scale
%                         longitudinal datasets. PLOS Genetics
%                         https://doi.org/10.1371/journal.pgen.1012184
% 
...
%                                       * 'analytic' (default; diagonal / compound symmetry)
%                                       * 'unstructured'
% 
...
if ~exist('FamilyStruct', 'var') || isempty(FamilyStruct)
    ...
    indvec = sub2ind([length(iid) length(iid)],subvec1,subvec2);
...
beta_hat                                = zeros(num_X, num_y, precision);
[beta_se, zmat, ymat_hat, ymat_res]     = deal(zeros(size(beta_hat), precision));
[betacon_hat, betacon_se]               = deal(zeros(size(contrasts,1), num_y, precision));
binvec                                  = NaN(1, num_y);
...
for permi = 0:nperms
    ...
    for iter = 1:max(1,niter)
        ...
        Ws_fam      = cell(1, nfam);
        [betacon_hat, betacon_se] = deal(zeros(size(contrasts,1), num_y, precision));
        [beta_hat,    beta_se]    = deal(zeros(num_X, num_y, precision));
        if permi == 0
            coeffCovar = zeros(num_X, num_X, num_y, precision);
        end
