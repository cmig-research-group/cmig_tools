function [fstem_imaging, config_design, dirname_out, dirname_imaging, datatype, dataFile, extraArgs] = FEMA_parseInputs(varargin)
    ...
    logging('Loading parameters from JSON file: %s\n', fname_json);
    ...
    % if strcmpi(datatype,  'external')
    %     dirname_imaging = fullfile(dirname_imaging, 'dataset_mod.parquet');
    % end
    ...
    if ismember('A',  configFile.params.random)
        if isfield(configFile.params, 'dir_grm')
            extraArgs{end+1} = 'GRM_file';
            extraArgs{end+1} = configFile.params.dir_grm;
        else 
            error('GRM file is required when additive genetic effect is specified.');
        end
    end 
    % nperms
    ...
    % matType
    if isfield(configFile, 'matType')
        extraArgs{end+1} = 'matType';
        extraArgs{end+1} = configFile.matType;
    end
    
    % do parallel
    if isfield(configFile.params.advanced, 'doPar')
        extraArgs{end+1} = 'doPar';
        extraArgs{end+1} = configFile.params.advanced.doPar;
