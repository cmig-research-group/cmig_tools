function FEMA_DEAP_gencache(nfrac,dirname_cache,dirname_tabulated,dirname_voxelwise,dirname_out,fname_design)
  % fname_design
  % dirname_voxelwise - dependant on the data release v4, v5, v6, etc...
  % nfrac - need to pass to init
  %
  % dirname_out and dirname_tabulated we can remove?  
  %
  ...
  for measi = 1:length(measlist)
    ...
    % Should add dirname_cache and nfrac as arguments, get rid of dirname_out, dirname_tabulated? -- should write out imaging phenotypes only
    FEMA_DEAP_init(fstem_pheno_voxel,fname_design,dirname_out,dirname_tabulated,dirname_voxelwise_tmp,'voxel',dirname_cache,nfrac);
