function [beta_hat beta_se zmat logpmat ...
  ...
  sig2mat_perm logLikvec_perm binvec nvec_bins ...
  tvec_bins FamilyStruct] = FEMA_fit(X,iid,eid,fid,agevec,ymat,niter,contrasts,nbins,pihatmat,varargin)
%
...
% Create grid of normalized random effects -- currently supports only FSE models -- should generalize to arbitrary set of random effects
  ...
  FamilyStruct = struct('clusterinfo',{clusterinfo},'M',{M},'famtypevec',{famtypevec},'famtypelist',{famtypelist},'nfamtypes',nfamtypes,'iid',{iid},'fid',{fid},'iid_list',{iid_list},'fid_list',{fid_list},'nfam',nfam,...
                        'sig2grid',sig2grid,'sig2gridl',sig2gridl,'sig2gridu',sig2gridu,'sig2gridi',sig2gridi,'sig2gridind',sig2gridind,'nsig2bins',nsig2bins,'subvec1',subvec1,'subvec2',subvec2);
...
%                      disp(rowvec(sig2mat_ml(:, coli)/sum(sig2mat_ml(:, coli)))) 
                  ...
                  if strcmpi('IGLS',RandomEstType) % Using approach described by Goldstein and Lindquist

                     if 0 % ~isempty(sig2mat_true)
                       binvec = binvec_true; % Initialize with true bin, to see if that improves estimates -- doesn't seem to change things much
                     end
...
%                    lam = [1.0 0]; % This should be equivalent to standard MoM -- seems to work
                    ...
                    msemat_igls = NaN(size(sig2mat,1),nsig2bins); msemat_mom = NaN(size(sig2mat,1),nsig2bins);
                    muemat_igls = NaN(size(sig2mat,1),nsig2bins); muemat_mom = NaN(size(sig2mat,1),nsig2bins);
                    for sig2bini = binvec_unique
                      fprintf(1,'sig2bini=%d/%d (%s)\n',sig2bini,length(binvec_unique),datestr(now));
...
%                              tmp = LHS(fnumvec==ivec(ii),ivec_bin);
                      ...
                      if ~isempty(sig2mat_true)
                        ...
                        disp('IGLS')
                        disp(sqrt(nanmean((sig2mat_igls(:,ivec_bin)-sig2mat_true(:,ivec_bin)).^2,2)))
                        disp('MoM')
                        disp(sqrt(nanmean((sig2mat_mom(:,ivec_bin)-sig2mat_true(:,ivec_bin)).^2,2)))
                        muemat_igls(:,sig2bini) = nanmean((sig2mat_igls(:,ivec_bin)-sig2mat_true(:,ivec_bin)),2);
                        muemat_mom(:,sig2bini) = nanmean((sig2mat_mom(:,ivec_bin)-sig2mat_true(:,ivec_bin)),2);
                        msemat_igls(:,sig2bini) = nanmean((sig2mat_igls(:,ivec_bin)-sig2mat_true(:,ivec_bin)).^2,2);
                        msemat_mom(:,sig2bini) = nanmean((sig2mat_mom(:,ivec_bin)-sig2mat_true(:,ivec_bin)).^2,2);
                      end
                    ...
                    if ~isempty(sig2mat_true) % sig2mat_igls is much worse than sig2mat_mom for some bins E for bins with high E, F and S, for bins with high S, low F 
                      disp('IGLS')
                      disp(sqrt(nanmean((sig2mat_igls-sig2mat_true).^2,2)))
                      disp('MoM')
                      disp(sqrt(nanmean((sig2mat_mom-sig2mat_true).^2,2)))
                    end
                    figure(666);
                    for fi = 1:size(msemat_igls,1)
                      im_igls = zeros(nbins*ones(1,length(RandomEffects)-1)); im_igls(sig2gridind) = sqrt(msemat_igls(fi,:));
                      im_mom = zeros(nbins*ones(1,length(RandomEffects)-1)); im_mom(sig2gridind) = sqrt(msemat_mom(fi,:));
                      crange = nanmax([im_igls(:); im_mom(:)])*[-1 1];
                      subplot(size(msemat_igls,1),3,(fi-1)*3+1); im = im_igls; imagesc(im,crange); axis equal xy tight; colormap(blueblackred); colorbar; title(sprintf('rmse %s IGLS',RandomEffects{fi})); ylabel(RandomEffects{1}); xlabel(RandomEffects{2});
                      subplot(size(msemat_igls,1),3,(fi-1)*3+2); im = im_mom; imagesc(im,crange); axis equal xy tight; colormap(blueblackred); colorbar; title(sprintf('rmse %s MoM',RandomEffects{fi})); ylabel(RandomEffects{1}); xlabel(RandomEffects{2});
%                      crange = nanmax(colvec(im_igls-im_mom))*[-1 1];
                      subplot(size(msemat_igls,1),3,(fi-1)*3+3); im = im_igls-im_mom; imagesc(im,crange); axis equal xy tight; colormap(blueblackred); colorbar; title(sprintf('rmse %s IGLS-MoM',RandomEffects{fi})); ylabel(RandomEffects{1}); xlabel(RandomEffects{2});
                    end
                   figure(667);
                   for fi = 1:size(msemat_igls,1)
                     im_igls = zeros(nbins*ones(1,length(RandomEffects)-1)); im_igls(sig2gridind) = muemat_igls(fi,:);
                     im_mom = zeros(nbins*ones(1,length(RandomEffects)-1)); im_mom(sig2gridind) = muemat_mom(fi,:);
                     crange = nanmax(abs([im_igls(:); im_mom(:)]))*[-1 1];
                     subplot(size(msemat_igls,1),3,(fi-1)*3+1); im = im_igls; imagesc(im,crange); axis equal xy tight; colormap(blueblackred); colorbar; title(sprintf('bias %s IGLS',RandomEffects{fi})); ylabel(RandomEffects{1}); xlabel(RandomEffects{2});
                     subplot(size(msemat_igls,1),3,(fi-1)*3+2); im = im_mom; imagesc(im,crange); axis equal xy tight; colormap(blueblackred); colorbar; title(sprintf('bias %s MoM',RandomEffects{fi})); ylabel(RandomEffects{1}); xlabel(RandomEffects{2});
%                     crange = nanmax(colvec(im_igls-im_mom))*[-1 1];
                     subplot(size(msemat_igls,1),3,(fi-1)*3+3); im = abs(im_igls)-abs(im_mom); imagesc(im,crange); axis equal xy tight; colormap(blueblackred); colorbar; title(sprintf('bias %s IGLS-MoM',RandomEffects{fi})); ylabel(RandomEffects{1}); xlabel(RandomEffects{2});
                   end

% ax = gca; ax.XTickLabel = num2str(binvals_edges(ax.XTick)) % Should replace voxel nuumbers with binvals

% ToDo
%   Check if greater errpor in igls estimates for high S & low F are associated with error in covariance estimate
...
%   Does  inclduing higher-order moments improve things?
                  ...
                  if nseg == 1 % Should get rid of parallelized version
                        tic
                        [beta_hat beta_se betacon_hat betacon_se tvec_bins nvec_bins] = FEMA_sig2binseg_parfeval(X,ymat,contrasts,clusterinfo,binvec,sig2mat,sig2tvec,RandomEffects,GroupByFamType,nfamtypes,famtypevec,OLSflag,SingleOrDouble);
                        toc
...
logging('***Done*** (%0.2f seconds total -- %0.2f seconds net)\n',(now-starttime)*3600*24,(now-starttime-tshim)*3600*24);
