...
%% Get basic information from the overall cache file
% Nested function makes this code static and therefore I need to explicetly 
...
%% Get y variable information from this fraci file
...
ymat_bak = ymat_frac;
    ...
    function run_job(job_name, job_fstem_imaging)
        ...
        tic
        % This takes most of the time -- should use fread instead?
        taskinfo = load(fname_job_design);
        toc

        % Create ID list for matching
        idvec  = strcat(iid, '_', eid);
        idvec2 = strcat(taskinfo.iid, '_', taskinfo.eid);

        % PP: 2026-01-02:
        % Need to additionally re-order GRM, PregID, and HomeID
        % Does not take contrast
        % Need to account for additional outputs from FEMA_fit
        % Should take as input the JSON specification file from the
        % frontend and account for user parameters

        ymat_frac = ymat_bak;
        if ~isequal(idvec, idvec2)
            [~, IA, IB] = intersect(idvec, idvec2, 'stable');
            X           = taskinfo.X(IB,:);
            ymat        = ymat_frac(IA,:);
            % Should re-scompute FamilyStruct, etc.
        else
            X       = taskinfo.X;
            ymat    = ymat_frac;
            IA      = [1:length(idvec)];
        ...
        % Work with Pravesh to update to use integreated FEMA_DEAP_fit -- 
        % need to re-compute FamilyStruct if iid and eid change
        [beta_hat, beta_se, zmat, logpmat, sig2tvec, sig2mat] =       ...
         FEMA_fit(X, iid, eid(IA), fid(IA), agevec(IA), ymat,         ...
                  contrasts, nbins, GRM, 'FamilyStruct',FamilyStruct, ...
                  'RandomEffects', RandomEffects, 'HomeID', HomeID,   ...
                  'PregID', PregID); 
