function [colnames_model X] = FEMA_DEAP_wrapper_submit(fstem_imaging,fname_design,dirname_imaging,datatype,designid,dirname_jobs)
  if nargin < 5
      fprintf(1,'Usage: FEMA_DEAP_wrapper(fstem_imaging,fname_design,dirname_imaging,datatype)\n',mfilename);
  ...
  [ymat, iid_concat, eid_concat, ivec_mask, mask, colnames_imaging, pihat, preg, address] = FEMA_process_data(sprintf('-%s',fstem_imaging),dirname_imaging,datatype); % Look for way to speed this up (currently takes ~2.5 sec)
