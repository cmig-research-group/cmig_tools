function FEMA_DEAP_init(fstem_imaging,fname_design,dirname_out,dirname_tabulated,dirname_imaging,datatype,dirname_cache,nfrac,varargin)
%
...
fpaths_out = {};
for des=1:length(fname_design) % Should get rid of the multiple design matrix option
...
% Should update FEMA_fit to allow passing of pre-computed clusterinfo 
  [beta_hat beta_se zmat logpmat sig2tvec sig2mat Hessmat logLikvec beta_hat_perm beta_se_perm zmat_perm sig2tvec_perm sig2mat_perm logLikvec_perm binvec nvec_bins tvec_bins FamilyStruct] = ...
     FEMA_fit(X_resid,iid,eid,fid,agevec,ymat_resid,niter,contrasts,nbins, pihatmat,'RandomEffects',RandomEffects,'nperms',nperms,'CovType',CovType,'FixedEstType',FixedEstType,'RandomEstType',RandomEstType,...
...
% dirname_cache = '~/FEMA_cache';
  ...
  if ~exist(dirname_cache,'dir')
      disp(sprintf('Creating cache directory %s',dirname_cache))
      jsystem(sprintf("mkdir -p %s", dirname_cache));
  end
  fname_cache = sprintf('%s/%s_%s.mat',dirname_cache,datatype,fstem_imaging);
  tic 
  save(fname_cache,'iid','eid','fid','agevec','pihatmat','RandomEffects','PregID','HomeID','FamilyStruct');
  toc

  for fraci = 1:nfrac
    ivec_tmp = find(tsum>=fraclist(fraci)&tsum<fraclist(fraci+1));
    ivec_frac = find(ismember(binvec,ivec_bins(ivec_tmp)));
    ymat_frac = ymat_resid(:,ivec_frac);
    fprintf(1,'fraci=%d/%d: length(ivec_tmp)=%d length(ivec_frac)=%d sumt=%f\n',fraci,nfrac,length(ivec_tmp),length(ivec_frac),sum(tvec_bins_pred(binvec(ivec_frac))));
    fname_out = sprintf('%s/%s_%s_%02d_%02d.mat',dirname_cache,datatype,fstem_imaging,fraci,nfrac);
    save(fname_out,'ymat_frac','ivec_frac','-v7.3');
    [beta_hat beta_se zmat logpmat sig2tvec sig2mat Hessmat logLikvec beta_hat_perm beta_se_perm zmat_perm sig2tvec_perm sig2mat_perm] = ...
      FEMA_fit(X_resid,iid,eid,fid,agevec,ymat_frac,niter,contrasts,nbins,pihatmat,'RandomEffects',RandomEffects,'nperms',nperms,'CovType',CovType,'FixedEstType',FixedEstType,'RandomEstType',RandomEstType,...
        'GroupByFamType',GroupByFamType,'NonnegFlag',NonnegFlag,'SingleOrDouble',SingleOrDouble,'logLikflag',logLikflag,'Hessflag',Hessflag,'ciflag',ciflag,...
        'permtype',permtype,'PregID',PregID,'HomeID',HomeID,'synthstruct',synthstruct);
% Should store actual timing info per fraction of bins, compare to prediction
  end
...
save(fname_task,'X_resid','iid','eid');
...
%   remove code for paralellization: parfeval
      ...
      elseif strcmpi(datatype, 'vertex')
            ...
            if contains(outputFormat, 'nifti') %FIXME: these are much smaller, so haven't added the same optimization as for voxelwise
                  ...
                  else % Write out in FreeSurfer curv format, with naming consistent with volumes
