...
makeDesign <- function(nda, outfile, time, contvar=NULL, catvar=NULL,  delta=NULL, interact=NULL, subjs=NULL, demean=FALSE, quadratic=NULL, mediator=NULL) {
  ...
  allvars<-c(contvar,catvar,delta)
	...
	# calculate deltas
	if ( !is.null(delta)) {
		idx_time1 <- grep(time[1], nda$eventname)
		idx_time2 <- grep(time[2], nda$eventname)
		for (d in delta) {
			vec0 <- rep(NA,dim(nda)[1]);
			vecD <- rep(NA,dim(nda)[1]);
			for (i in 1:length(idx_time)) {
				idx_subj <- which(nda$src_subject_id==subjid[i])
				idx_subj1 <- which(nda$eventname[idx_subj]==time[1])
				idx_subj1 <- idx_subj[idx_subj1]
				idx_subj2 <- which(nda$eventname[idx_subj]==time[2])
				idx_subj2 <- idx_subj[idx_subj2]
				# baseline
				if ( !is_empty(idx_subj1) ) {
					vec0[idx_subj1] <- nda[idx_subj1,d]
					vec0[idx_subj2] <- nda[idx_subj1,d]
				} else if ( is_empty(idx_subj1) & !is_empty(idx_subj2) ) {
					vec0[idx_subj2] <- nda[idx_subj2,d]
				}
				# delta
				if ( !is_empty(idx_subj1) & !is_empty(idx_subj2) ) {
					varD <- nda[idx_subj2,d] - nda[idx_subj1,d]
					vecD[idx_subj1] <- 0
					vecD[idx_subj2] <- varD
				} else if ( is_empty(idx_subj1) & !is_empty(idx_subj2) ) {
					vecD[idx_subj2] <- 0
